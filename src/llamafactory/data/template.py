...
def get_template_and_fix_tokenizer(tokenizer: "PreTrainedTokenizer", data_args: "DataArguments") -> "Template":
    ...
    try:
        ...
        if data_args.template:
            template_dir = "/home/work/chatbot-llms/huiyu.chen/ShopLLMs/Service/template"
            os.makedirs(template_dir, exist_ok=True)
            template_path = os.path.join(template_dir, f"{data_args.template}.jinja")
            with open(template_path, "w") as f:
                f.write(tokenizer.chat_template)

            logger.info(f"Saved Jinja template to {template_path}")
    except ValueError:
